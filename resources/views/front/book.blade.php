...
@section('tittle', 'Book Now!')
@section('content')
<section id="Content" class="bg-portto-black flex min-h-screen">
        <div class="w-[660px] min-h-screen flex flex-col p-[30px_40px] justify-end overflow-hidden bg-[url('{{ asset('assets/images/background/side-image.png') }}')] bg-cover bg-center bg-no-repeat">
            <div class="flex flex-col bg-white p-[30px] gap-5 rounded-[30px] w-[580px]">
                <div class="flex h-10 items-start overflow-hidden">
                    <img src="{{ asset('assets/images/logos/logo-testi5.svg') }}" class="h-full object-contain" alt="photo">
                </div>
        ...
        <div class="flex flex-col gap-[50px] items-center justify-center mx-auto py-4 bg-[url('{{ asset('assets/images/Ellipse.svg') }}')] bg-center bg-no-repeat bg-[length:540px]">
            ...
            <form method="POST" action="{{ route('front.book.store') }}" class="flex flex-col gap-5 w-[550px]">
                ...
                <div class="grid grid-cols-2 gap-5">
                    <label class="flex flex-col gap-[10px] font-semibold w-full">
                        ...
                        <select name="category" id="category" class="font-semibold bg-white rounded-full p-[14px_30px] pr-[54px] appearance-none outline-none focus:ring-[3px] focus:ring-portto-green invalid:text-[#878C9C] invalid:font-normal bg-[url(../assets/images/icons/arrow-down.svg)] bg-no-repeat bg-[91%]" required>
